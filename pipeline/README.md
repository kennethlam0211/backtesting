...
python -m pipeline.data_preprocessing --freq 5      # 3. bar features (optional; preview only for now)
```
...
```bash
python -m pipeline.raw_data_preprocessing                                   # all sessions
...
| Writes | `--out`, default `data/ES_trades_concat.parquet`: one row per merged tick with `ts`, `price` (ticks, x4), `volume`, `rth`, `session`, `hour`, `news_*` |
| Options | `--start`, `--end`: first / last session date, inclusive (default: all) |
| Before running | `data/` must exist (`mkdir -p data`); the script does not create it |
